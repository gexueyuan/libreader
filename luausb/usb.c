...
BINDING(get_port_numbers)
{
	...
	result = libusb_get_port_numbers(dev, portNums, sizeof(portNums));
	if (result < 0)
	{
		return lua__usberror(L, result);
	}

	lua_pushlstring(L, (char*)portNums, result);
	return 1;
}
...
/*ljc add by 2017-2-22*/
struct _hotplug_user_data {
	int mark;
	int ref;
	int handle;
	lua_State *L;
};
typedef struct _hotplug_user_data _hotplug_user_data;
static int LIBUSB_CALL _hotplug_callback(libusb_context *ctx, libusb_device *dev, libusb_hotplug_event event, void *user_data)
{
	_hotplug_user_data* ud = (_hotplug_user_data*)user_data;
	if (ud->mark == 0x706C7567)
	{
		uint8_t portNums[8];
		int portNumsLen = libusb_get_port_numbers(dev, portNums, sizeof(portNums));
		if (portNumsLen > 0)
		{
			lua_rawgeti(ud->L, LUA_REGISTRYINDEX, ud->ref);
			lua_pushinteger(ud->L, libusb_get_bus_number(dev));
			lua_pushlstring(ud->L, (char*)portNums, portNumsLen);
			lua_pushinteger(ud->L, event);
			lua_pcall(ud->L, 3, 0, 0);
		}
	}else if(ctx){
	}
	return 0;
...
BINDING(hotplug_register_callback)
{
	int ref, result;
	_hotplug_user_data* ud;

	int argc = lua_gettop(L);

	libusb_context* ctx = luausb_opt_context(L, 1, NULL);
	libusb_hotplug_event events = (argc > 2)? luaL_optinteger(L, 2, LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED|LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT) : (LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED|LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT);
	libusb_hotplug_flag flags = (argc > 3)? luaL_optinteger(L, 3, 0) : 0;
	int vendor_id = (argc > 4)? luaL_optinteger(L, 4, LIBUSB_HOTPLUG_MATCH_ANY) : LIBUSB_HOTPLUG_MATCH_ANY;
	int product_id = (argc > 5)? luaL_optinteger(L, 5, LIBUSB_HOTPLUG_MATCH_ANY) : LIBUSB_HOTPLUG_MATCH_ANY;
	int dev_class = (argc > 6)? luaL_optinteger(L, 6, LIBUSB_HOTPLUG_MATCH_ANY) : LIBUSB_HOTPLUG_MATCH_ANY;

	ref = luaL_ref(L, LUA_REGISTRYINDEX);

	ud = (_hotplug_user_data*)lua_newuserdata(L, sizeof(_hotplug_user_data));
	ud->mark = 0x706C7567; /*plug*/
	ud->ref = ref;
	ud->L = L;
	ud->handle = 0;

	result = libusb_hotplug_register_callback(ctx, events, flags, vendor_id, product_id, dev_class, _hotplug_callback, (void*)ud, &(ud->handle));
	if (result < 0)
	{
		return lua__usberror(L, result);
	}
...
BINDING(hotplug_deregister_callback)
{
	libusb_context* ctx = luausb_opt_context(L, 1, NULL);
	_hotplug_user_data* ud = (_hotplug_user_data*)lua_touserdata(L, 2);
	
	if (ud->mark == 0x706C7567)
	{	
		libusb_hotplug_deregister_callback(ctx, ud->handle);
...
struct luaL_Reg libusb_context__methods[] = {
	...
	BIND(set_debug)
	BIND(hotplug_register_callback)
	BIND(hotplug_deregister_callback)	
	{0, 0},
};
/*
